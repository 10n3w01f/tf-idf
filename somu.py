...
def file_read():
    ...
    #     text_dict = {}
    i = 0
    filename = ("C:/Users/sampa/Documents/dataset/movie-reviews/Dataset_Copy/2012")
    #    print("**********************"+directory+"*******************")
    os.chdir(filename)
...
def filter_dict(filtered_text, parsed_data, doc_id):
    filter_dict = {}
    count = 0
    for word in filtered_text:
        filter_dict['required_word'] = word
        if word in parsed_data:
            filter_dict['doc_id'] = doc_id
            break
            count += 1
            filter_dict['frequency'] = count

        filtered_data.append(filter_dict)
    with open(r'C:\Users\sampa\Documents\datasetfile.txt', mode='w') as out:
        out.write(str(filter_dict))

    return filtered_data


def dict_to_file(dictionary):
    with open('file.txt', mode='w') as out:
...
class PreProcess:
    # text = "I have to say this movie is very \n tense. The disasters in // it make you think, will the/n world's
    # really end this /n way? It's one of those films " \ "where //it gets your mind thinking about // the world
    # around us and how natural disasters can and will happen." #     discard_html(text)
    ...
    filter_text = ['recommend', 'movie', 'good', 'drama', 'plot', 'awesome', 'dislike', 'cgi', 'fantastic', 'stupid']
    # filtered
    for dict in text:
        text = dict['doc_data']
        ...
        parsed_data = remove_apostrophe(parsed_data)
        parsed_data = remove_a_character(parsed_data)
        parsed_data = tokenize_data(parsed_data)
        parsed_data = lemmatize_data(parsed_data)
        filtered_data = filter_dict(filter_text, parsed_data, dict['doc_id'])
        #print(filtered_data)
        #dict_to_file(filtered_data)
    # for dict_data in filtered_data:
    #     print(dict_data[])
    #     filtered_data=dict_data['doc']
    #     filtered_data = sent_tokenize(filtered_data)
    #     # print(type(parsed_data))
    #     filtered_data = [remove_all_special_character(data) for data in filtered_data]
    #     doc_info = get_doc_info(filtered_data)
    #     freq_info = create_freq(filtered_data)
